...
actor class EXTNFT(init_owner : Principal) = this {
  ...
  private stable var data_failedDisbursementQueueState : [(TokenIndex, AccountIdentifier, SubAccount, Nat64)] = [];
  ...
  private stable var data_failedPaymentSettlementsTableState : [(AccountIdentifier, Payment)] = [];
  ...
  private stable var data_loggger : [Text] = [];
  ...
  var _failedDisbursements : Queue.Queue<(TokenIndex, AccountIdentifier, SubAccount, Nat64)> = Queue.fromArray(data_failedDisbursementQueueState);
  ...
  var _failedPaymentSettlements : HashMap.HashMap<AccountIdentifier, Payment> = HashMap.fromIter(data_failedPaymentSettlementsTableState.vals(), 0, AID.equal, AID.hash);
  ...
  let MAX_CHUNK_STORAGE : Nat = 2800000000; //2800MB
  ...
  system func preupgrade() {
    ...
    data_failedPaymentSettlementsTableState := Iter.toArray(_failedPaymentSettlements.entries());
    data_saleGroupsTableState := Iter.toArray(_saleGroups.entries());
    data_disbursementQueueState := Queue.toArray(_disbursements);
    data_failedDisbursementQueueState := Queue.toArray(_failedDisbursements);
  ...
  system func postupgrade() {
    ...
    data_failedPaymentSettlementsTableState := [];
    data_saleGroupsTableState := [];
    data_disbursementQueueState := [];
    data_failedDisbursementQueueState := [];
  ...
  public shared (msg) func heartbeat_external() : async () {
    ...
    if (data_internalRunHeartbeat == true) {
      heartbeatCount += 1;
      try {
        if (heartbeatCount % 100000 == 10000) {
          ignore heartbeat_failedDisbursements();
          ignore heartbeat_assetCanisters();

        };
        ignore heartbeat_capEvents();
        ignore heartbeat_paymentSettlements();
        ignore heartbeat_disbursements();
  ...
  public shared (msg) func heartbeat_disbursements() : async () {
    data_loggger := Array.append(data_loggger, ["Run disbursements"]);
        ...
        case (?d) {
          ...
          if (d.3 != 0) {
            try {
              ...
              _failedDisbursements := Queue.add(d, _failedDisbursements);
            };
          };
        };
        case (_) {};
  ...
  public shared (msg) func heartbeat_failedDisbursements() : async () {
    data_loggger := Array.append(data_loggger, ["Run failed disbursements"]);
    while (Queue.size(_failedDisbursements) > 0 and data_internalRunHeartbeat) {
      var last = Queue.next(_failedDisbursements);
      switch (last.0) {
        case (?d) {
          _failedDisbursements := last.1;
          if (d.3 != 0) {
            try {
              var bh = await ExternalService_ICPLedger.send_dfx({
                memo = 0;
  ...
  public query func getFailedDisbursementsSize() : async Nat {
    Queue.size(_failedDisbursements);
  };

  public query func getHeapSize<system>() : async Nat {
    return Prim.rts_heap_size();
  };

  public query func getDataLogger() : async [Text] {
    data_loggger;
  };

  public query func getFailedDisbursements() : async [(TokenIndex, AccountIdentifier, SubAccount, Nat64)] {
    let arr : [(TokenIndex, AccountIdentifier, SubAccount, Nat64)] = Queue.toArray(_failedDisbursements);
    arr;
  };

  public query func getFailedPaymentSettlements() : async [(AccountIdentifier, Payment)] {
    return Iter.toArray(_failedPaymentSettlements.entries());
  };

  public shared (msg) func heartbeat_paymentSettlements() : async Result.Result<(), CommonError> {
    for ((paymentAddress, settlement) in _expiredPaymentSettlements().vals()) {
      data_loggger := Array.append(data_loggger, ["Run payment settlemets"]);
      switch (settlement.purchase) {
        case (#nft t) {
          try {
            ignore (await ext_marketplaceSettle(paymentAddress));
          } catch (e) {
            data_loggger := Array.append(data_loggger, ["Error at " # debug_show (t) # " with payment address " # paymentAddress]);
            _removeFromExpiredPaymentSettlements(paymentAddress);
            _addToFailedPaymentSettlements(paymentAddress, settlement);

          };
        };
        case (#sale q) {
          ignore (ext_saleSettle(paymentAddress));
        };
        case (_) {};
      };
    };
  ...
  func _removeFromExpiredPaymentSettlements(paymentAddress : AccountIdentifier) {
    (_paymentSettlements.delete(paymentAddress));
  };

  func _removeFromFailedPaymentSettlements(paymentAddress : AccountIdentifier) {
    (_failedPaymentSettlements.delete(paymentAddress));
  };

  func _addToFailedPaymentSettlements(paymentAddress : AccountIdentifier, settlement : Payment) {
    (_failedPaymentSettlements.put(paymentAddress, settlement));
  };
  ...
  public query func heartbeat_pending() : async [(Text, Nat)] {
    [
      ...
      // ("Failed Disbursements", Queue.size(_failedDisbursements)),
      ...
      case (?(settlement, response)) {
        switch (response) {
          case (#ok ledgerResponse) {
            if (ledgerResponse.e8s < 10000) {
              return #err(#Other("Disbursement amount too small"));
              _paymentSettlements.delete(paymentaddress);
            };
            switch (settlement.purchase) {
              case (#nft token) {
    ...
    switch (await ext_checkPayment(paymentaddress)) {
      case (?(settlement, response)) {
        switch (response) {
          case (#ok ledgerResponse) {
            if (ledgerResponse.e8s < 10000) {
              return #err("Disbursement amount too small");
              _paymentSettlements.delete(paymentaddress);
            };
            switch (settlement.purchase) {
              case (#sale quantity) {
  ...
  func _getSubAccountForAddress(a : AccountIdentifier) : ?SubAccount {
    ...
    while (s < (4294967296 + data_internalNextSubAccount)) {
  ...
  func _transferTokenToUserQuick(tindex : TokenIndex, receiver : AccountIdentifier) {
    _registry.put(tindex, receiver);
    _addToUserTokens(tindex, receiver);
  };
          ...
          case (?saleCurrent) {
            ...
            for (a in existingGroups.vals()) {
              if (name == a.name) {
                ...
                newGroups := Array.append(newGroups, [{ name = a.name; limit = a.limit; start = a.start; end = a.end; pricing = a.pricing; participants = newPart }]);
              } else {
      ...
      case (?g) {
        switch (data_saleCurrent) {
          case (?saleCurrent) {
            ...
            for (a in existingGroups.vals()) {
              if (name == a.name) {
                newGroups := Array.append(newGroups, [{ name = a.name; limit = a.limit; start = a.start; end = a.end; pricing = newpricing; participants = a.participants }]);
              } else {
                newGroups := Array.append(newGroups, [a]);
              };
            };
      ...
      case (?g) {
        switch (data_saleCurrent) {
          case (?saleCurrent) {
            ...
            for (a in existingGroups.vals()) {
              if (name == a.name) {
                newGroups := Array.append(newGroups, [{ name = a.name; limit = a.limit; start = newStart; end = newEnd; pricing = a.pricing; participants = a.participants }]);
              } else {
                newGroups := Array.append(newGroups, [a]);
              };
            };
  ...
  // for(t in _tokenMetadata.entries()){
  //   switch(t.1){
  //     case(#nonfungible md) {
  //       if (md.thumbnail == "") {
  //         _tokenMetadata.put(t.0, #nonfungible({
  //           name = md.name;
  //           asset = md.asset;
  //           thumbnail = md.asset#"_thumbnail";
